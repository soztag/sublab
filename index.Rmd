...
imap_dfr(.x = scores, .id = "condition", .f = function(x, y) {
  ...
  ggplot(mapping = aes(x = score, y = item, colour = component, shape = condition)) +
  geom_point() + 
...
score_range <- range(scores) %>% 
  ...
  max() %>% 
  c(-., .)
scores_df <- cbind(scores$desirable, scores$probable)
...
invoke(.f = gridExtra::grid.arrange, .x = gs)
```
