...
# because sth in pensieve (?) is very slow, this can only be done once
sorts <- fs::path("raw", "2017-10-zaw40") %>% 
  ...
  map_dfr(.f = function(x) {
    ...
    # TODO this transformation should really happen here, but currently fails, 
    # blocked b/c of https://github.com/tidyverse/purrr/issues/538
    # sort <- as_psClosedSorts(
    #   obj = sort,
    #   items = english
    # )
    # validate grid and items with validate
    tibble(
      name = name,
      time = time,
      condition = condition,
...
sorts <- arrange(.data = sorts, name, condition, time) %>% 
  group_by(name, condition) %>% 
  dplyr::filter(time == last(time)) %>% 
  ungroup()

# transform to canonical form
sorts$sort <- plyr::laply(.data = sorts$sort, .fun = function(x) {
  as_psClosedSorts(obj = x, items = english, grid = grids$zaw17)
})
...
ts18 <- map_dfr(
  ...
  .f = function(x) {
    ...
    condition <- stringr::str_split(string = filename, pattern = "_", simplify = TRUE)[1,2] %>% 
      fs::path_ext_remove() %>% 
      unclass()
    sort <- readr::read_csv(file = x, col_names = FALSE) %>% 
      as.matrix() %>% 
      as_psSort()
    validate_S3(x = sort, grid = grids$ts_18, items = english)
...
ts18$sort <- plyr::laply(.data = ts18$sort, .fun = function(x) {
  as_psClosedSorts(obj = x, grid = grids$ts_18, items = english)
})
sorts <- rbind(sorts, ts18)
```
