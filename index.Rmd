...
```{r distro}
distro <- qmethod::make.distribution(nstat = nrow(concourse), max.bin = 5)
```
...
glitchies <- readxl::excel_sheets(path = "raw/screenshots/screenshots.xlsx")[-c(1,2)]
screenshots <- sapply(X = glitchies, simplify = FALSE, USE.NAMES = TRUE, FUN = function(x) {
  res <- readxl::read_xlsx(path = "raw/screenshots/screenshots.xlsx", sheet = x, range = "B1:L7", col_names = FALSE)
...
for (cond in names(qres)) {  # co for conditions
  for (p in colnames(qres[[cond]])) {  # p for people
    ...
    for (i in rownames(qres[[cond]])) {  # i for items
      ...
      sorts[i, pname, cond] <- qres[[cond]][i, p]
    }  
...
sorts[,"Renate", "desirable"] <- meltsorts(x = screenshots$`7b`, lookup = concourse[, c("number", "handle")])[,1]
sorts[,"Christian", "desirable"] <- meltsorts(x = screenshots$`23a`, lookup = concourse[, c("number", "handle")])[,1]
sorts[,"Christiane", "probable"] <- meltsorts(x = screenshots$`sabine-wahrscheinlich`, lookup = concourse[, c("number", "handle")])[,1]
sorts[,"Linda", "probable"] <- meltsorts(x = screenshots$`5a`, lookup = concourse[, c("number", "handle")])[,1]
sorts[,"Tony", "probable"] <- meltsorts(x = screenshots$`19a`, lookup = concourse[, c("number", "handle")])[,1]
sorts[,"Lars", "probable"] <- meltsorts(x = screenshots$`12b`, lookup = concourse[, c("number", "handle")])[,1]
...
# add zaw17 participants
zaw17ap <- list.files(path = "raw/2017-10-zaw40/")
zaw17ap <- strsplit(x = zaw17ap, split = "-")
zaw17ap <- unlist(zaw17ap)
zaw17ap <- unique(zaw17ap)
zaw17ap <- zaw17ap[zaw17ap != "desirable" & zaw17ap != "probable"]

newar <- sorts
newar[,,] <- NA
newar <- newar[,1:length(zaw17ap),]
dimnames(newar)$people <- zaw17ap
sorts <- abind::abind(sorts, newar, along = 2, use.dnns = TRUE)
all(unique(dimnames(sorts)$people) == unique(dimnames(sorts)$people))

newpeeps <- data.frame(userid = zaw17ap,
                       agegroup = NA,
                       gender = NA,
                       group = "zaw17",
                       selfid = TRUE,
                       name = zaw17ap)
peoplevars <- rbind(peoplevars, newpeeps)

for (peep in zaw17ap) {
  for (cond in c("desirable", "probable")) {
    # TODO age and gender are missing
    # TODO gender and age should be checked for consistency between two sorts
    thispath <- file.path(
      "raw/2017-10-zaw40/",
      paste0(peep, "-", cond),
      cond)
    alltimes <- list.files(path = thispath)
    alltimes <- tools::file_path_sans_ext(alltimes)
    alltimes <- alltimes[!(alltimes %in% c("age", "gender"))] 
    latest <- alltimes[which(max(strptime(x = alltimes, format = "%Y_%m_%d_%H_%M_%S")) == strptime(x = alltimes, format = "%Y_%m_%d_%H_%M_%S"))]
    thissortpath <- file.path(thispath, paste0(latest, ".csv"))
    if (file.exists(thissortpath)) {
      thissort <- read.csv(
        file = thissortpath,
        header = TRUE,
        stringsAsFactors = FALSE
      )
      for (it in concourse$handle) {
        thissort <- as.matrix(thissort)
        if (it %in% thissort) {
          index <- which(x = it == thissort, arr.ind = TRUE)
          if (index[1,"row"] %% 2 == 0) {  # when even
            xval <- index[1,"row"]
          } else {
            xval <- index[1,"row"] + 0.5
          }
          sorts[it, peep, cond] <- xval
        }
      }
    }
  }
...
# cleanup
rm(list = ls()[!(ls() %in% c("peoplevars", "comments", "concourse", "sorts", "distro"))])

# fix names
dimnames(sorts)$people <- lettercase::make_names(names = dimnames(sorts)$people)
peoplevars$name <- lettercase::make_names(names = peoplevars$name)
```
...
plot(cor$des, use_js = FALSE) + ggtitle("Ähnlichkeit der Subjektivität der Teilnehmenden (Wünschenswert)")
plot(cor$prob, use_js = FALSE) + ggtitle("Ähnlichkeit der Subjektivität der Teilnehmenden (Wahrscheinlich)")
```
...
paranres$screeplot + ggtitle("Kriterien zur Faktorgüte (Screeplot und Parallelanalyse)")
...
colnames(alloas)[1:4] <- c("wuenschenswert", "wahrscheinlich-1", "wahrscheinlich-2", "wahrscheinlich-3")

allscores <- cbind(zwares$desirable$zsc, zwares$probable$zsc, kurz = concourse$kurz, text = concourse$german)
colnames(allscores)[1:4] <- c("wuenschenswert", "wahrscheinlich-1", "wahrscheinlich-2", "wahrscheinlich-3")

loa1 <- ggplot(data = alloas, mapping = aes(x = wuenschenswert, y = `wahrscheinlich-1`, label = names, shape = gruppe))
loa1 <- loa1 + geom_point(color = "blue")
loa1 <- loa1 + geom_text_repel()
dev.new()
loa1

loa2 <- ggplot(data = alloas, mapping = aes(x = wuenschenswert, y = `wahrscheinlich-2`, label = names, shape = gruppe))
loa2 <- loa2 + geom_point(color = "blue")
loa2 <- loa2 + geom_text_repel()
dev.new()
loa2

loa3 <- ggplot(data = alloas, mapping = aes(x = wuenschenswert, y = `wahrscheinlich-3`, label = names, shape = gruppe))
loa3 <- loa3 + geom_point(color = "blue")
loa3 <- loa3 + geom_text_repel()
dev.new()
loa3

score1 <- ggplot(data = allscores, mapping = aes(x = wuenschenswert, y = `wahrscheinlich-1`, label = text))
score1 <- score1 + geom_point(color = "blue")
score1 <- score1 + geom_text_repel()
score1
score2 <- ggplot(data = allscores, mapping = aes(x = wuenschenswert, y = `wahrscheinlich-2`, label = text))
score2 <- score2 + geom_point(color = "red")
score2 <- score2 + geom_text_repel()
score2
dev.new()
score3 <- ggplot(data = allscores, mapping = aes(x = wuenschenswert, y = `wahrscheinlich-3`, label = text))
score3 <- score3 + geom_point(color = "green")
score3 <- score3 + geom_text_repel()
score3
